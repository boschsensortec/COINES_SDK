@echo off
echo Starting APP3.0 firmware update process...
echo.
...
%DFU_PATH%\dfu-util --device -,108c:ab3d -a FLASH -D coines_bridge\coines_bridge_flash_firmware.bin -R
...
%APP_SWITCH_PATH%\app_switch usb_dfu_bl
%DFU_PATH%\dfu-util --device -,108c:ab3d -a RAM -D bootloader_update\usb_ble_dfu_bootloader.pkg -R
...
%APP_SWITCH_PATH%\app_switch usb_dfu_bl
%DFU_PATH%\dfu-util --device -,108c:ab3d -a RAM -D mtp_fw_update\usb_mtp.pkg -R
echo USB MTP firmware flashed successfully.
echo Waiting for device reset...
echo.

echo -----------------------------------------------------------
echo APP3.0 firmware update process completed successfully!
