...
%DFU_PATH%\dfu-util --device -,108c:4b3d -a FLASH -D coines_bridge\coines_bridge_flash_firmware.bin -R
...
%APP_SWITCH_PATH%\app_switch usb_dfu_bl
%DFU_PATH%\dfu-util --device -,108c:4b3d -a RAM -D bootloader_update\usb_ble_dfu_bootloader.pkg -R
...
%APP_SWITCH_PATH%\app_switch usb_dfu_bl
%DFU_PATH%\dfu-util --device -,108c:4b3d -a RAM -D mtp_fw_update\usb_mtp.pkg -R
